...
def generate_matrix_visualizations(
    ...
    _create_adjacency_matrix(
        small_framework,
        session_path,
        "small_adjacency_matrix",
        "Small Dataset - Adjacency Matrix",
        colors,
    )
    _create_adjacency_matrix(
        large_framework,
        session_path,
        "large_adjacency_matrix",
        "Large Dataset - Adjacency Matrix",
        colors,
    )
    ...
    _create_correlation_matrix(
        ...
        "Small Dataset - Correlation Matrix",
        colors,
    )
    _create_correlation_matrix(
        large_framework,
        ...
        "Large Dataset - Correlation Matrix",
        colors,
    )

    # Generate strength matrices
    _create_strength_matrix(
        small_framework,
        session_path,
        "small_strength_matrix",
        "Small Dataset - Relationship Strength Matrix",
        colors,
    )
    _create_strength_matrix(
        large_framework,
        session_path,
        "large_strength_matrix",
        "Large Dataset - Relationship Strength Matrix",
        colors,
    )

    # Generate comprehensive matrix overview
    _create_matrix_overview(
        small_framework,
        large_framework,
        session_path,
        "comprehensive_matrix_overview",
        "Comprehensive Matrix Overview",
        colors,
    )
...
def _create_adjacency_matrix(
    ...
    for rel in framework._relationships.values():
        ...
        for pattern in all_patterns:
            if (
                (isinstance(pattern, Property) and rel.property_id == pattern.id)
                or (isinstance(pattern, Process) and rel.process_id == pattern.id)
                or (isinstance(pattern, Perspective) and rel.perspective_id == pattern.id)
            ):
    ...
    if np.any(adj_matrix > 0):
        sns.heatmap(
            ...
            yticklabels=[p.name[:8] for p in all_patterns],
            square=True,
        )
    ...
    if np.any(type_matrix > 0):
        sns.heatmap(
            ...
            yticklabels=["Properties", "Processes", "Perspectives"],
            square=True,
        )
        ax2.set_title("Inter-Type Connections", fontsize=12, fontweight="bold")
    else:
        ax2.text(
            0.5, 0.5, "No inter-type connections", ha="center", va="center", transform=ax2.transAxes
        )
        ax2.set_title("Inter-Type Connections", fontsize=12, fontweight="bold")
...
def _create_correlation_matrix(
    ...
    for corr_type, values in type_correlations.items():
        if values:
            ...
            ax3.barh(
                y_pos, mean_corr, height=0.05, alpha=0.7, label=f"{corr_type}: {mean_corr:.3f}"
            )
            ax3.text(mean_corr + 0.01, y_pos, f"{len(values)} connections", fontsize=8, va="center")
            y_pos -= 0.08
...
def _create_matrix_overview(
    ...
    small_density = (
        len(small_framework._relationships) / (small_patterns * (small_patterns - 1) / 2)
        if small_patterns > 1
        else 0
    )
    large_density = (
        len(large_framework._relationships) / (large_patterns * (large_patterns - 1) / 2)
        if large_patterns > 1
        else 0
    )
