...
def generate_3d_cube_visualizations(
    ...
    _create_3d_p3if_cube(
        small_framework, session_path, "small_3d_cube", "Small Dataset - P3IF 3D Cube", colors
    )
    _create_3d_p3if_cube(
        large_framework, session_path, "large_3d_cube", "Large Dataset - P3IF 3D Cube", colors
    )

    # Generate dimension cubes
    _create_p3if_dimension_cube(
        small_framework,
        session_path,
        "small_dimension_cube",
        "Small Dataset - P3IF Dimension Cube",
        colors,
    )
    _create_p3if_dimension_cube(
        large_framework,
        session_path,
        "large_dimension_cube",
        "Large Dataset - P3IF Dimension Cube",
        colors,
    )

    # Generate relationship cubes
    _create_relationship_cubes(
        small_framework,
        session_path,
        "small_relationship_cubes",
        "Small Dataset - Relationship Cubes",
        colors,
    )
    _create_relationship_cubes(
        large_framework,
        session_path,
        "large_relationship_cubes",
        "Large Dataset - Relationship Cubes",
        colors,
    )


def _create_3d_p3if_cube(
...
def _draw_relationship_lines(
    ...
    prop_id_to_pos = {
        p.id: pos
        for p, pos in zip(
            [p for p in framework._patterns.values() if isinstance(p, Property)], prop_positions
        )
    }
    proc_id_to_pos = {
        p.id: pos
        for p, pos in zip(
            [p for p in framework._patterns.values() if isinstance(p, Process)], proc_positions
        )
    }
    persp_id_to_pos = {
        p.id: pos
        for p, pos in zip(
            [p for p in framework._patterns.values() if isinstance(p, Perspective)], persp_positions
        )
    }
...
def _create_p3if_dimension_cube(
    framework: P3IFFramework, session_path: Path, filename: str, title: str, colors: Dict[str, str]
):
    ...
    _draw_dimension_focus(
        ax1, "Properties", properties, processes, perspectives, cube_size, colors, focus_axis="x"
    )

    # Dimension 2: Processes (Y-axis focus)
    _draw_dimension_focus(
        ax2, "Processes", properties, processes, perspectives, cube_size, colors, focus_axis="y"
    )

    # Dimension 3: Perspectives (Z-axis focus)
    _draw_dimension_focus(
        ax3, "Perspectives", properties, processes, perspectives, cube_size, colors, focus_axis="z"
    )
...
def _draw_dimension_focus(
    ...
    if focus_axis == "x":
        ...
        ax.set_zlabel(
            "Perspectives (P)", fontsize=12, fontweight="bold", color=colors["perspective"]
        )
...
def _create_relationship_cubes(
    framework: P3IFFramework, session_path: Path, filename: str, title: str, colors: Dict[str, str]
):
    ...
    _draw_relationship_connections(
        ax, framework, colors, prop_positions, proc_positions, persp_positions
    )
...
def _draw_small_cube(ax, x: float, y: float, z: float, size: float, color: str, alpha: float):
    """Draw a small cube centered at (x, y, z) inside the 3D axis."""
    r = [-size / 2, size / 2]
