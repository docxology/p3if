...
from p3if.core.analysis.network import NetworkAnalyzer
...
class NetworkVisualizer(Visualizer):
    ...
    def visualize_full_network(
        ...
        if color_by == "type":
            ...
            node_colors = [
                type_colors.get(G.nodes[n].get("type", "unknown"), "#cccccc") for n in G.nodes
            ]
        elif color_by == "domain":
            domain_colors = self.get_domain_colors()
            node_colors = [
                domain_colors.get(G.nodes[n].get("domain", "unknown"), "#cccccc") for n in G.nodes
            ]
    ...
    def visualize_communities(
        ...
        if not communities.get("communities"):
            self.logger.warning(
                f"No communities found for {graph_type} graph using {algorithm} algorithm"
            )
            return None
        ...
        node_colors = [
            community_color_map.get(node_community.get(node, -1), "#cccccc") for node in G.nodes
        ]
    ...
    def visualize_centrality(
        ...
        nodes = nx.draw_networkx_nodes(
            ...
            cmap=plt.cm.viridis if graph_type == "domain" else None,
        )
