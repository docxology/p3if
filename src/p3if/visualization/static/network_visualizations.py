...
def generate_network_visualizations(
    ...
    _create_general_network_graph(
        small_framework, session_path, "small_network", "Small P3IF Network", colors
    )
    _create_general_network_graph(
        large_framework, session_path, "large_network", "Large P3IF Network", colors
    )
...
def _create_general_network_graph(
    ...
    for pattern_id, pattern in framework._patterns.items():
        G.add_node(pattern_id, name=pattern.name, type=pattern.type.value, domain=pattern.domain)
    ...
    for rel_id, relationship in framework._relationships.items():
        ...
        for i in range(len(connected_patterns)):
            for j in range(i + 1, len(connected_patterns)):
                G.add_edge(
                    connected_patterns[i],
                    connected_patterns[j],
    ...
    legend_elements = [
        plt.Line2D(
            ...
            marker="o",
            color="w",
            markerfacecolor=colors["property"],
            markersize=10,
            label="Properties",
        ),
        plt.Line2D(
            ...
            marker="o",
            color="w",
            markerfacecolor=colors["process"],
            markersize=10,
            label="Processes",
        ),
        plt.Line2D(
            ...
            marker="o",
            color="w",
            markerfacecolor=colors["perspective"],
            markersize=10,
            label="Perspectives",
        ),
    ]
...
def _create_component_specific_networks(
    framework: P3IFFramework, session_path: Path, colors: Dict[str, str]
):
    """Create networks specific to each P3IF component type."""
    # Properties network
    _create_component_network(
        framework, session_path, "properties", "property", "Properties Network", colors
    )

    # Processes network
    _create_component_network(
        framework, session_path, "processes", "process", "Processes Network", colors
    )

    # Perspectives network
    _create_component_network(
        framework, session_path, "perspectives", "perspective", "Perspectives Network", colors
    )


def _create_component_network(
    ...
    for pattern_id, pattern in framework._patterns.items():
        if pattern.type.value == pattern_type:
            G.add_node(
                pattern_id, name=pattern.name, type=pattern.type.value, domain=pattern.domain
            )
...
def _create_domain_specific_networks(
    framework: P3IFFramework, session_path: Path, colors: Dict[str, str]
):
...
def _create_domain_network(
    framework: P3IFFramework, session_path: Path, domain: str, colors: Dict[str, str]
):
    ...
    if len(G.nodes()) == 0:
        ax.text(
            0.5, 0.5, f"No patterns in {domain}", ha="center", va="center", transform=ax.transAxes
        )
    else:
        # Layout
        if len(G.nodes()) < 15:
