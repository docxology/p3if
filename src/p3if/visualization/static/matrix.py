...
class MatrixVisualizer(Visualizer):
    ...
    def visualize_relationship_matrix(
        ...
        """
        if pattern_type_x not in ["property", "process", "perspective"] or pattern_type_y not in [
            "property",
            "process",
            "perspective",
        ]:
        ...
        if len(patterns_x) > max_patterns:
            self.logger.info(
                f"Limiting {pattern_type_x} patterns from {len(patterns_x)} to {max_patterns}"
            )
            ...
            patterns_x = sorted(
                ...
                key=lambda p: sum(
                    1
                    for r in self.framework._relationships.values()
                    if (p.type == "property" and r.property_id == p.id)
                    or (p.type == "process" and r.process_id == p.id)
                    or (p.type == "perspective" and r.perspective_id == p.id)
                ),
                reverse=True,
            )[:max_patterns]
        ...
        if len(patterns_y) > max_patterns:
            self.logger.info(
                f"Limiting {pattern_type_y} patterns from {len(patterns_y)} to {max_patterns}"
            )
            patterns_y = sorted(
                patterns_y,
                key=lambda p: sum(
        ...
        if use_progressive_rendering and matrix_is_large:
            self.logger.info(
                f"Using progressive rendering for large matrix ({len(patterns_x)}x{len(patterns_y)} = {matrix_size} cells)"
            )
        ...
        ax.set_title(
            f"Relationship Matrix: {pattern_type_x.capitalize()} × {pattern_type_y.capitalize()}"
        )
    ...
    def visualize_domain_similarity(
        self,
        ...
        if len(domains) > max_domains:
            ...
            domains = sorted(domains, key=lambda d: domain_pattern_counts.get(d, 0), reverse=True)[
                :max_domains
            ]
    ...
    def visualize_pattern_correlation(
        self,
        file_path: Union[str, Path] = None,
        ...
        for i, pattern1 in enumerate(patterns):
            for j, pattern2 in enumerate(patterns):
                ...
                for rel in self.framework._relationships.values():
                    if (
                        (
                            rel.property_id == pattern1.id
                            and (rel.process_id == pattern2.id or rel.perspective_id == pattern2.id)
                        )
                        or (
                            rel.process_id == pattern1.id
                            and (
                                rel.property_id == pattern2.id or rel.perspective_id == pattern2.id
                            )
                        )
                        or (
                            rel.perspective_id == pattern1.id
                            and (rel.property_id == pattern2.id or rel.process_id == pattern2.id)
                        )
                    ):
    ...
    def visualize_relationship_matrix_progressive(
        ...
        for i, pattern_x in enumerate(patterns_x):
            for j, pattern_y in enumerate(patterns_y):
                ...
                for rel in framework._relationships.values():
                    ...
                    x_match = (
                        (pattern_type_x == "property" and rel.property_id == pattern_x.id)
                        or (pattern_type_x == "process" and rel.process_id == pattern_x.id)
                        or (pattern_type_x == "perspective" and rel.perspective_id == pattern_x.id)
                    )

                    y_match = (
                        (pattern_type_y == "property" and rel.property_id == pattern_y.id)
                        or (pattern_type_y == "process" and rel.process_id == pattern_y.id)
                        or (pattern_type_y == "perspective" and rel.perspective_id == pattern_y.id)
                    )
        ...
        sns.heatmap(
            matrix,
            annot=len(patterns_x) * len(patterns_y)
            <= 400,  # Only annotate if matrix is small enough
            fmt=".2f",
            cmap=cmap,
            xticklabels=x_labels,
            yticklabels=y_labels,
