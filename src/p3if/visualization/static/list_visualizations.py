...
def generate_list_visualizations(
    ...
    _create_properties_list(
        small_framework, session_path, "small_properties_list", "Small Dataset - Properties", colors
    )
    _create_processes_list(
        small_framework, session_path, "small_processes_list", "Small Dataset - Processes", colors
    )
    _create_perspectives_list(
        small_framework,
        session_path,
        "small_perspectives_list",
        "Small Dataset - Perspectives",
        colors,
    )

    # Generate lists for large dataset
    _create_properties_list(
        large_framework, session_path, "large_properties_list", "Large Dataset - Properties", colors
    )
    _create_processes_list(
        large_framework, session_path, "large_processes_list", "Large Dataset - Processes", colors
    )
    _create_perspectives_list(
        large_framework,
        session_path,
        "large_perspectives_list",
        "Large Dataset - Perspectives",
        colors,
    )
...
def _create_properties_list(
    ...
    for prop in properties:
        ...
        table_data.append(
            [prop.name, prop.domain, description, tags_str, f"{prop.quality_score:.2f}"]
        )
...
def _create_processes_list(
    ...
    for proc in processes:
        description = (
            proc.description[:50] + "..." if len(proc.description) > 50 else proc.description
        )
        complexity = getattr(proc, "complexity", "medium")
        automation = getattr(proc, "automation_level", "manual")

        table_data.append([proc.name, proc.domain, description, complexity, automation])
...
def _create_perspectives_list(
    ...
    # Create table
    table_data = [
        ["Perspective Name", "Domain", "Viewpoint", "Stakeholder Type", "Expertise Level"]
    ]
    ...
    for persp in perspectives:
        ...
        table_data.append([persp.name, persp.domain, viewpoint, stakeholder_type, expertise_level])
...
def _create_comprehensive_overview(
    ...
    # Properties overview
    properties_small = len(
        [p for p in small_framework._patterns.values() if isinstance(p, Property)]
    )
    properties_large = len(
        [p for p in large_framework._patterns.values() if isinstance(p, Property)]
    )

    ax1.bar(
        ["Small Dataset", "Large Dataset"],
        [properties_small, properties_large],
        color=[colors["property"], colors["property"]],
        alpha=0.7,
    )
    ...
    ax2.bar(
        ["Small Dataset", "Large Dataset"],
        [processes_small, processes_large],
        color=[colors["process"], colors["process"]],
        alpha=0.7,
    )
