...
def generate_grid_visualizations(
    ...
    _create_p3if_component_grid(
        small_framework,
        session_path,
        "small_p3if_grid",
        "Small Dataset - P3IF Component Grid",
        colors,
    )
    _create_p3if_component_grid(
        large_framework,
        session_path,
        "large_p3if_grid",
        "Large Dataset - P3IF Component Grid",
        colors,
    )

    # Generate relationship grid
    _create_relationship_grid(
        small_framework,
        session_path,
        "small_relationship_grid",
        "Small Dataset - Relationship Grid",
        colors,
    )
    _create_relationship_grid(
        large_framework,
        session_path,
        "large_relationship_grid",
        "Large Dataset - Relationship Grid",
        colors,
    )

    # Generate domain grid
    _create_domain_grid(
        small_framework, session_path, "small_domain_grid", "Small Dataset - Domain Grid", colors
    )
    _create_domain_grid(
        large_framework, session_path, "large_domain_grid", "Large Dataset - Domain Grid", colors
    )

    # Generate comprehensive P3IF matrix
    _create_comprehensive_p3if_matrix(
        small_framework,
        large_framework,
        session_path,
        "comprehensive_p3if_matrix",
        "Comprehensive P3IF Matrix",
        colors,
    )


def _create_p3if_component_grid(
...
def _draw_main_p3if_grid(
    ax, properties: list, processes: list, perspectives: list, colors: Dict[str, str]
):
    ...
    # Draw title
    ax.text(
        0.5, 0.95, "P3IF Component Grid", ha="center", va="center", fontweight="bold", fontsize=14
    )
    ...
    for section_name, components, color, row in sections:
        ...
        for i, component in enumerate(components[:4]):  # Show up to 4 components
            ...
            ax.add_patch(
                patches.Circle((x + cell_width / 2, comp_y), 0.03, facecolor=color, alpha=0.8)
            )
            ax.text(
                x + cell_width / 2 + 0.05,
                comp_y,
...
def _draw_component_pair_grid(
    ...
    for i in range(n1):
        for j in range(n2):
            ...
            ax.add_patch(
                patches.Rectangle(
                    (x, y), 0.15, 0.1, facecolor="lightgray", alpha=0.5, edgecolor="black"
                )
            )
    ...
    ax.text(
        0.5,
        0.05,
        f"{n1} × {n2} = {n1 * n2} possible combinations",
        ha="center",
        va="center",
        fontsize=8,
    )
...
def _add_grid_summary(ax, framework: P3IFFramework, colors: Dict[str, str]):
    ...
    for text, color in summary_data:
        if text:
            ax.add_patch(patches.Rectangle((0.1, y_pos), 0.8, 0.08, facecolor=color, alpha=0.8))
            ax.text(
                0.5, y_pos + 0.04, text, ha="center", va="center", fontweight="bold", fontsize=10
            )
        y_pos -= 0.1


def _create_relationship_grid(
    framework: P3IFFramework, session_path: Path, filename: str, title: str, colors: Dict[str, str]
):
    ...
    strengths = [
        float(rel.strength) if hasattr(rel.strength, "__float__") else 0.5
        for rel in framework._relationships.values()
    ]
    ...
    confidences = [
        float(rel.confidence) if hasattr(rel.confidence, "__float__") else 1.0
        for rel in framework._relationships.values()
    ]
...
def _create_domain_grid(
    ...
    for i, domain in enumerate(domains):
        ...
        domain_processes = [
            p for p in framework._patterns.values() if p.domain == domain and isinstance(p, Process)
        ]
        domain_perspectives = [
            p
            for p in framework._patterns.values()
        ...
        if domain_properties:
            ax.add_patch(
                patches.Circle((x + 0.05, pattern_y), 0.02, facecolor=colors["property"], alpha=0.8)
            )
            ax.text(x + 0.08, pattern_y, f"P: {len(domain_properties)}", fontsize=8)

        if domain_processes:
            ax.add_patch(
                patches.Circle((x + 0.15, pattern_y), 0.02, facecolor=colors["process"], alpha=0.8)
            )
        ...
        if domain_perspectives:
            ax.add_patch(
                patches.Circle(
                    (x + 0.05, pattern_y - 0.03), 0.02, facecolor=colors["perspective"], alpha=0.8
                )
            )
        ...
        for prop in domain_properties[:2]:
            ...
            ax.text(
                x + 0.125, pattern_y + 0.02, prop.name[:10], fontsize=7, ha="center", va="center"
            )
            pattern_y -= 0.05
            patterns_added += 1
        ...
        for proc in domain_processes[:2]:
            ...
            ax.text(
                x + 0.125, pattern_y + 0.02, proc.name[:10], fontsize=7, ha="center", va="center"
            )
            pattern_y -= 0.05
            patterns_added += 1
        ...
        for persp in domain_perspectives[:2]:
            ax.add_patch(
                patches.FancyBboxPatch(
                    ...
                    0.04,
                    boxstyle="round,pad=0.01",
                    facecolor=colors["perspective"],
                    alpha=0.7,
                )
            )
            ax.text(
                x + 0.125, pattern_y + 0.02, persp.name[:10], fontsize=7, ha="center", va="center"
            )
            pattern_y -= 0.05
            patterns_added += 1
...
def _create_comprehensive_p3if_matrix(
    small_framework: P3IFFramework,
    ...
    small_matrix = [
        ...
        [
            small_stats["prop_persp_rels"],
            small_stats["proc_persp_rels"],
            small_stats["perspectives"],
        ],
    ]
    ...
    large_matrix = [
        ...
        [
            large_stats["prop_persp_rels"],
            large_stats["proc_persp_rels"],
            large_stats["perspectives"],
        ],
    ]

    # Small dataset matrix
    ax1.imshow(small_matrix, cmap="Reds", alpha=0.8)
    ...
    for i in range(len(components)):
        for j in range(len(components)):
            ax1.text(
                j, i, small_matrix[i][j], ha="center", va="center", color="black", fontweight="bold"
            )

    # Large dataset matrix
    ax2.imshow(large_matrix, cmap="Blues", alpha=0.8)
    ...
    for i in range(len(components)):
        for j in range(len(components)):
            ax2.text(
                j, i, large_matrix[i][j], ha="center", va="center", color="white", fontweight="bold"
            )
    ...
    ax3.bar(
        x - width,
        [small_stats["total_patterns"], large_stats["total_patterns"]],
        width,
    ...
    ax3.bar(
        x,
        [small_stats["total_relationships"], large_stats["total_relationships"]],
        width,
    ...
    ax3.bar(
        x + width,
        [small_stats["domains"], large_stats["domains"]],
        width,
        label="Domains",
