...
def generate_statistical_visualizations(
    ...
    _create_component_distributions(
        small_framework,
        large_framework,
        session_path,
    ...
    _create_quality_metrics_analysis(
        small_framework,
        large_framework,
        session_path,
        "quality_metrics",
    ...
    _create_relationship_statistics(
        small_framework,
        large_framework,
        session_path,
        "relationship_stats",
    ...
    _create_domain_analysis(
        small_framework, large_framework, session_path, "domain_analysis", "Domain Analysis", colors
    )

    # Generate comprehensive statistical overview
    _create_statistical_overview(
        small_framework,
        large_framework,
        session_path,
        "comprehensive_stats",
...
def _create_component_distributions(
    ...
    small_persps = len(
        [p for p in small_framework._patterns.values() if isinstance(p, Perspective)]
    )
    ...
    large_persps = len(
        [p for p in large_framework._patterns.values() if isinstance(p, Perspective)]
    )
    ...
    ax1.bar(
        x + width, persp_counts, width, label="Perspectives", color=colors["perspective"], alpha=0.8
    )
    ...
    if small_total > 0:
        ax2.pie(
            [small_props, small_procs, small_persps],
    ...
    if large_total > 0:
        ax3.pie(
            [large_props, large_procs, large_persps],
            labels=["Properties", "Processes", "Perspectives"],
            colors=[colors["property"], colors["process"], colors["perspective"]],
            autopct="%1.1f%%",
            startangle=90,
    ...
    ax4.stackplot(
        datasets,
        [prop_counts, proc_counts, persp_counts],
        labels=["Properties", "Processes", "Perspectives"],
        colors=[colors["property"], colors["process"], colors["perspective"]],
        alpha=0.8,
    )
...
def _create_quality_metrics_analysis(
    small_framework: P3IFFramework,
    large_framework: P3IFFramework,
    session_path: Path,
    filename: str,
    ...
    for framework, scores_list in [
        (small_framework, small_quality_scores),
        (large_framework, large_quality_scores),
    ]:
    ...
    if large_quality_scores:
        ax2.hist(
            large_quality_scores, bins=10, alpha=0.7, color=colors["process"], label="Large Dataset"
        )
        ax2.axvline(
            np.mean(large_quality_scores),
            color="blue",
            linestyle="--",
            alpha=0.8,
    ...
    if small_quality_scores and large_quality_scores:
        ...
        ax3.bar(
            ...
            yerr=stds,
            capsize=5,
            color=[colors["property"], colors["process"]],
            alpha=0.8,
        )
...
def _create_relationship_statistics(
    small_framework: P3IFFramework,
    large_framework: P3IFFramework,
    session_path: Path,
    filename: str,
    ...
    if small_strengths:
        ax1.hist(
            small_strengths, bins=10, alpha=0.7, color=colors["property"], label="Small Dataset"
        )
        ax1.axvline(np.mean(small_strengths), color="red", linestyle="--", alpha=0.8)

    if large_strengths:
        ax2.hist(
            large_strengths, bins=10, alpha=0.7, color=colors["process"], label="Large Dataset"
        )
    ...
    if small_confidences:
        ax3.hist(
            small_confidences, bins=10, alpha=0.7, color=colors["property"], label="Small Dataset"
        )
        ax3.axvline(np.mean(small_confidences), color="red", linestyle="--", alpha=0.8)

    if large_confidences:
        ax4.hist(
            large_confidences, bins=10, alpha=0.7, color=colors["process"], label="Large Dataset"
        )
...
def _create_domain_analysis(
    ...
    for framework, domain_dict in [
        (small_framework, small_domains),
        (large_framework, large_domains),
    ]:
        for pattern in framework._patterns.values():
    ...
    ax2.bar(
        ...
        [small_diversity, large_diversity],
        color=[colors["property"], colors["process"]],
        alpha=0.8,
    )
    ...
    if small_domains:
        ...
        for domain, counts in list(small_domains.items())[:5]:  # Show top 5
            ...
            small_summary.append(
                [domain, total, counts["properties"], counts["processes"], counts["perspectives"]]
            )

        table = ax3.table(
            cellText=small_summary,
...
def _create_statistical_overview(
    small_framework: P3IFFramework,
    large_framework: P3IFFramework,
    session_path: Path,
    filename: str,
    ...
    ax1.bar(
        x - width / 2, pattern_counts, width, label="Patterns", color=colors["property"], alpha=0.8
    )
    ax1.bar(
        x + width / 2,
        relationship_counts,
    ...
    small_components = [
        small_stats["properties"],
        small_stats["processes"],
        small_stats["perspectives"],
    ]
    large_components = [
        large_stats["properties"],
        large_stats["processes"],
    ...
    # Add value labels
    ax3.text(
        0, small_quality + 0.01, f"{small_quality:.2f}", ha="center", va="bottom", fontweight="bold"
    )
    ax3.text(
        1, large_quality + 0.01, f"{large_quality:.2f}", ha="center", va="bottom", fontweight="bold"
    )
