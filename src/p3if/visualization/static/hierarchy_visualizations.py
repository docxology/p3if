...
def generate_hierarchical_visualizations(
    ...
    _create_p3if_hierarchy(
        small_framework,
        session_path,
        "small_p3if_hierarchy",
        "Small Dataset - P3IF Hierarchy",
        colors,
    )
    _create_p3if_hierarchy(
        large_framework,
        session_path,
        "large_p3if_hierarchy",
        "Large Dataset - P3IF Hierarchy",
        colors,
    )

    # Generate domain hierarchies
    _create_domain_hierarchy(
        small_framework,
        session_path,
        "small_domain_hierarchy",
        "Small Dataset - Domain Hierarchy",
        colors,
    )
    _create_domain_hierarchy(
        large_framework,
        session_path,
        "large_domain_hierarchy",
        "Large Dataset - Domain Hierarchy",
        colors,
    )

    # Generate component type hierarchies
    _create_component_type_hierarchy(
        small_framework,
        session_path,
        "small_component_hierarchy",
        "Small Dataset - Component Type Hierarchy",
        colors,
    )
    _create_component_type_hierarchy(
        large_framework,
        ...
        "Large Dataset - Component Type Hierarchy",
        colors,
    )

    # Generate relationship hierarchies
    _create_relationship_hierarchy(
        small_framework,
        session_path,
        "small_relationship_hierarchy",
        "Small Dataset - Relationship Hierarchy",
        colors,
    )
    _create_relationship_hierarchy(
        large_framework,
        session_path,
        "large_relationship_hierarchy",
        "Large Dataset - Relationship Hierarchy",
        colors,
    )


def _create_p3if_hierarchy(
...
def _draw_p3if_tree(
    ...
    ax.text(
        0.5, 1.5, "P3IF", ha="center", va="center", fontweight="bold", color="white", fontsize=10
    )

    # Properties branch
    ax.add_patch(Rectangle((0.2, 1.0), 0.6, 0.2, color=colors["property"], alpha=0.8))
    ax.text(
        0.5, 1.1, f"Properties ({len(properties)})", ha="center", va="center", fontweight="bold"
    )
    ...
    ax.text(
        0.5, 0.3, f"Perspectives ({len(perspectives)})", ha="center", va="center", fontweight="bold"
    )
...
def _draw_domain_org_chart(ax, framework: P3IFFramework, colors: Dict[str, str]):
    ...
    for i, domain in enumerate(domains):
        ...
        if domain_perspectives:
            ax.add_patch(
                Circle((x_pos, y_pos - 0.05), 0.03, color=colors["perspective"], alpha=0.8)
            )
            ax.text(
                x_pos + 0.05,
                y_pos - 0.05,
                f"Persp: {len(domain_perspectives)}",
                fontsize=8,
                ha="left",
                va="center",
            )
...
def _draw_relationship_tree(ax, framework: P3IFFramework, colors: Dict[str, str]):
        ...
        if rel.property_id:
            prop_name = next(
                (
                    p.name
                    for p in framework._patterns.values()
                    if p.id == rel.property_id and isinstance(p, Property)
                ),
                "Unknown",
            )
        ...
        if rel.process_id:
            proc_name = next(
                (
                    p.name
                    for p in framework._patterns.values()
                    if p.id == rel.process_id and isinstance(p, Process)
                ),
                "Unknown",
            )
        ...
        if rel.perspective_id:
            persp_name = next(
                (
                    p.name
                    for p in framework._patterns.values()
                    if p.id == rel.perspective_id and isinstance(p, Perspective)
                ),
                "Unknown",
            )
    ...
    if rel_nodes:
        nx.draw_networkx_nodes(
            G, pos, nodelist=rel_nodes, node_color="lightgray", node_shape="s", node_size=200, ax=ax
        )
    ...
    if persp_nodes:
        nx.draw_networkx_nodes(
            G,
            pos,
            nodelist=persp_nodes,
            node_color=colors["perspective"],
            node_size=150,
            alpha=0.8,
            ax=ax,
        )
...
def _draw_hierarchy_summary(
    ...
    for label, value, color in summary_data:
        ax.add_patch(
            FancyBboxPatch(
                (0.1, y_pos), 0.8, 0.15, boxstyle="round,pad=0.02", facecolor=color, alpha=0.8
            )
        )
...
def _create_domain_hierarchy(
    ...
    for i, domain in enumerate(domains):
        ...
        ax.text(
            0.5, y_pos + 0.075, domain, ha="center", va="center", fontweight="bold", fontsize=12
        )
        ...
        if domain_properties:
            ...
            ax.text(
                x_pos + 0.03,
                y_pos - 0.03,
                f"Properties: {len(domain_properties)}",
                fontsize=9,
                ha="left",
                va="center",
            )
        ...
        if domain_processes:
            ...
            ax.text(
                x_pos + 0.03,
                y_pos - 0.03,
                f"Processes: {len(domain_processes)}",
                fontsize=9,
                ha="left",
                va="center",
            )
        ...
        if domain_perspectives:
            ax.add_patch(
                Circle((x_pos, y_pos - 0.03), 0.02, color=colors["perspective"], alpha=0.9)
            )
            ax.text(
                x_pos + 0.03,
                y_pos - 0.03,
                f"Perspectives: {len(domain_perspectives)}",
                fontsize=9,
                ha="left",
                va="center",
            )
...
def _create_component_type_hierarchy(
    framework: P3IFFramework, session_path: Path, filename: str, title: str, colors: Dict[str, str]
):
    ...
    levels = [
        ...
        (
            "Component Types",
            f"Properties ({len(properties)}), Processes ({len(processes)}), Perspectives ({len(perspectives)})",
            0.6,
        ),
        (
            "Individual Components",
            f"Total: {len(properties + processes + perspectives)} components",
            0.4,
        ),
    ]
    ...
    y_pos = 0.3
    for comp_type, comp_list, color in [
        ("Properties", properties, colors["property"]),
        ...
        for i, comp in enumerate(comp_list[:3]):  # Show first 3
            ...
            ax.text(
                0.25 + i * 0.25, comp_y + 0.02, comp.name[:10], fontsize=7, ha="center", va="center"
            )
...
def _create_relationship_hierarchy(
    framework: P3IFFramework, session_path: Path, filename: str, title: str, colors: Dict[str, str]
):
