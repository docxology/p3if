...
def generate_animation_visualizations(
    ...
    _create_p3if_rotation_animation(
        small_framework,
        large_framework,
        session_path,
        "p3if_components_rotation",
        "P3IF Components Rotation",
        colors,
    )

    # Generate framework evolution animation
    _create_framework_evolution_animation(
        small_framework,
        large_framework,
        session_path,
        "framework_evolution",
    ...
    _create_relationship_dynamics_animation(
        small_framework,
        large_framework,
        session_path,
    ...
    _create_component_interaction_animation(
        small_framework,
        large_framework,
        session_path,
...
def _create_p3if_rotation_animation(
    ...
    for frame in range(n_frames):
        ...
        for i, (name, color, small_count, large_count) in enumerate(components):
            ...
            # Component label
            ax.text(
                x, y, name, ha="center", va="center", fontweight="bold", fontsize=10, color="white"
            )
        ...
        ax.set_title(
            f"{title} - Frame {frame + 1}/{n_frames}", fontsize=14, fontweight="bold", pad=20
        )
...
def _create_framework_evolution_animation(
    small_framework: P3IFFramework,
    large_framework: P3IFFramework,
    session_path: Path,
    filename: str,
    ...
    for frame in range(n_frames):
        ...
        ax.set_title(
            f"{title} - Evolution Step {frame + 1}/{n_frames}", fontsize=14, fontweight="bold"
        )
        ax.set_xlim(-1, 1)
        ax.set_ylim(-0.5, 0.5)
...
def _draw_framework_snapshot(
    ax, framework: P3IFFramework, label: str, x_offset: float, colors: Dict[str, str], alpha: float
):
    ...
    ax.add_patch(
        Rectangle(
            (x_offset - 0.15, 0.1), box_width, box_height, color=colors["property"], alpha=alpha
        )
    )
    ax.text(
        x_offset - 0.075,
    ...
    ax.add_patch(
        Rectangle(
            (x_offset - 0.15, 0.0), box_width, box_height, color=colors["process"], alpha=alpha
        )
    )
    ax.text(
        x_offset - 0.075,
    ...
    ax.add_patch(
        Rectangle(
            (x_offset - 0.15, -0.1), box_width, box_height, color=colors["perspective"], alpha=alpha
        )
    )
    ax.text(
        x_offset - 0.075,
        -0.05,
        f"Persp: {perspectives}",
        ha="center",
        va="center",
        fontsize=8,
        fontweight="bold",
    ...
    ax.text(
        x_offset + 0.1,
        0.05,
        f"Total: {len(framework._patterns)}",
        ha="center",
        va="center",
        fontweight="bold",
        fontsize=10,
...
def _create_relationship_dynamics_animation(
    small_framework: P3IFFramework,
    large_framework: P3IFFramework,
    session_path: Path,
    filename: str,
...
def _draw_p3if_triangle(ax, colors: Dict[str, str]):
    ...
    # Triangle vertices
    vertices = [(-1, -1.5), (2, -1.5), (0.5, 2)]  # Properties  # Processes  # Perspectives
    ...
    for i, (x, y) in enumerate(vertices):
        ...
        ax.text(
            x, y, labels[i], ha="center", va="center", fontweight="bold", color="white", fontsize=9
        )


def _draw_relationship_connections(
    ax, framework: P3IFFramework, colors: Dict[str, str], max_rels: int, frame: int
):
...
def _create_component_interaction_animation(
    small_framework: P3IFFramework,
    large_framework: P3IFFramework,
    session_path: Path,
    filename: str,
...
def _draw_evolving_interactions(
    ax, framework: P3IFFramework, colors: Dict[str, str], frame: int, n_frames: int
):
    ...
    if progress > 0.8:
        ...
        n_rels = min(
            int(rel_progress * len(framework._relationships)), len(framework._relationships)
        )
        ...
        for i, rel in enumerate(list(framework._relationships.values())[:n_rels]):
            ...
            ax.plot(
                [-2.5, -0.5, 1.5], [0.5, 0.5, 0.5], color="gray", alpha=alpha * 0.5, linewidth=2
            )
