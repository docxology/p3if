...
class Visualizer:
    ...
    def save_figure(
        ...
        if total_pixels > 16000000:  # 4000 x 4000 pixels
            ...
            self.logger.info(
                f"Large figure detected ({fig_width:.1f}x{fig_height:.1f} inches). Reducing DPI from {self.dpi} to {adjusted_dpi}"
            )
        else:
        ...
        try:
            ...
            if save_completed:
                ...
                self.logger.warning(
                    f"Figure save operation timed out after {MAX_SAVE_TIME} seconds, trying with lower quality: {path}"
                )
    ...
    def setup_figure(
        self, nrows: int = 1, ncols: int = 1, figsize: Optional[Tuple[float, float]] = None
