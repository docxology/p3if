...
class NetworkAnalyzer:
            ...
            for rel_id, rel in self.framework._relationships.items():
                ...
                for i in range(len(patterns)):
                    for j in range(i + 1, len(patterns)):
                        G.add_edge(
                            patterns[i], patterns[j], weight=rel.strength, relationship_id=rel_id
                        )
    ...
    def get_centrality_measures(
        self, graph_type: str = "full", top_n: int = 10
        ...
        for measure_name, measure_func in centrality_functions.items():
            try:
                ...
                for node_id, value in sorted_nodes[:top_n]:
                    ...
                    # Add node attributes
                    if graph_type == "full" or (
                        graph_type == "bipartite" and not str(node_id).startswith("rel_")
                    ):
    ...
    def get_communities(
        self, graph_type: str = "full", algorithm: str = "louvain"
    ) -> Dict[str, Any]:
        """
        ...
        try:
            if algorithm == "louvain":
                try:
                    ...
                    self.logger.warning(
                        "Failed to import 'community' module. Using label propagation instead."
                    )
        ...
        for i, community in enumerate(communities):
            ...
            formatted_communities.append(
                {"id": i, "size": len(community), "nodes": community_nodes}
            )
