...
class MetaAnalyzer:
    ...
    def _calculate_domain_similarity_matrix(self, domains: Set[str]) -> List[List[float]]:
        ...
        patterns_by_domain = {
            domain: self.domain_manager.get_patterns_by_domain(domain) for domain in domain_list
        }
        ...
        for i in range(n):
            for j in range(n):
                ...
                for pattern_type in ["property", "process", "perspective"]:
                    names_i.update(
                        p.name.lower() for p in patterns_by_domain[domain_i][pattern_type]
                    )
                    names_j.update(
                        p.name.lower() for p in patterns_by_domain[domain_j][pattern_type]
                    )
    ...
    def get_cross_domain_relationships(self) -> Dict[str, Any]:
        ...
        if not cross_domain_rels:
            return {"count": 0, "domain_pairs": [], "strongest_connections": []}
        ...
        strongest_connections = sorted(
            cross_domain_rels, key=lambda x: x["strength"], reverse=True
        )[
            :10
    ...
    def get_pattern_correlation_matrix(self) -> Dict[str, Any]:
        ...
        return {"features": corr_columns, "correlation_matrix": correlation_matrix}
