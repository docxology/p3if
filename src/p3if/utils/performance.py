...
class LRUCache:
    ...
    def __getitem__(self, key: str):
        """Get item from cache (for dict-like access)."""
        return self.get(key)

    def __setitem__(self, key: str, value: Any):
        """Set item in cache (for dict-like access)."""
        self.put(key, value)

    def __delitem__(self, key: str):
        """Delete item from cache (for dict-like access)."""
        with self.lock:
...
def cached(cache_key: str = None, ttl: Optional[float] = None):
    ...
    # If called as @cached, return the decorator directly
    # If called as @cached(), return the decorator function
    if callable(cache_key):
        # @cached was used directly on a function
        return actual_decorator(cache_key)
    else:
        # @cached() was used with parameters
