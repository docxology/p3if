...
class SyntheticDataGenerator:
    ...
    def __init__(self, domain_data_path: Optional[Union[str, Path]] = None):
        ...
        if domain_data_path:
            ...
            if domains_dir.exists() and index_file.exists():
                ...
                # Start from the file location and go up until we find a directory with data/domains
                current = Path(__file__).parent
                project_root = None
                for _ in range(5):  # Try up to 5 levels up
                    candidate = current / "data" / "domains"
                    if candidate.exists():
                ...
                if domains_dir.exists() and index_file.exists():
                    ...
                    self.logger.warning(
                        f"No domain data found. Checked: {domains_dir}, {default_data_path}"
                    )
    ...
    def load_domain_index(self, index_path: Union[str, Path]) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Loaded domain index with {len(self.domain_index.get('domains', []))} domains"
            )
    ...
    def load_domain_data(self, data_path: Union[str, Path]) -> None:
        ...
        if path.is_dir():
            ...
            self.logger.warning(
                f"Path {data_path} is not a directory. Legacy file loading has been removed."
            )
    ...
    def get_domain_info(self, domain_name: str) -> Dict[str, Any]:
        ...
        if self.domain_index:
            # Try to match by ID first (normalize domain name to match index format)
            normalized_name = domain_name.lower().replace(" ", "").replace("_", "")
            for domain_info in self.domain_index.get("domains", []):
                index_id = domain_info.get("id", "")
                index_name_normalized = (
                    domain_info.get("name", "").lower().replace(" ", "").replace("_", "")
                )
                if index_id == normalized_name or index_name_normalized == normalized_name:
                    return {
                        "name": domain_info.get("name", domain_name),
                        "id": domain_info.get("id", normalized_name),
                        "counts": domain_info.get(
                            "counts",
                            {
    ...
    def generate_for_domain(
        ...
        for prop_name in domain.get("properties", []):
            prop = Property(
                name=prop_name, description=f"Property: {prop_name}", domain=domain_name
            )
        ...
        for proc_name in domain.get("processes", []):
            proc = Process(name=proc_name, description=f"Process: {proc_name}", domain=domain_name)
        ...
        for persp_name in domain.get("perspectives", []):
            persp = Perspective(
                ...
                domain=domain_name,
                viewpoint=f"view_{persp_name.lower().replace(' ', '_')}",
            )
        ...
        for _ in range(num_relationships):
            ...
            while (
                (prop is None and proc is None)
                or (prop is None and persp is None)
                or (proc is None and persp is None)
            ):
                prop = random.choice(properties) if properties else None
                proc = random.choice(processes) if processes else None
                persp = random.choice(perspectives) if perspectives else None
    ...
    def generate_multi_domain(
        ...
        relationships_per_domain: int = 100,
    ) -> None:
        """
    ...
    def generate_cross_domain_connections(
        self, framework: P3IFFramework, num_connections: int = 50, min_strength: float = 0.3
    ) -> None:
        """
        ...
        for _ in range(num_connections):
            ...
            all_domains = list(
                set(
                    list(properties_by_domain.keys())
            ...
            dimensions_present = sum(
                1 for dim in [property_id, process_id, perspective_id] if dim is not None
            )
    ...
    def generate_domain(
        self, framework: P3IFFramework, domain_name: str, num_relationships: int = 100
    ) -> None:
        """
        ...
        return self.generate_for_domain(
            framework=framework, domain_name=domain_name, num_relationships=num_relationships
        )
