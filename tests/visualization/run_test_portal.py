...
from p3if.visualization.portal import (  # noqa: E402 - after sys.path setup
    VisualizationPortal,
)  # noqa: E402 - import after sys.path setup
from p3if.utils.config import Config  # noqa: E402 - import after sys.path setup

# Import test fixtures
from tests.fixtures import (  # noqa: E402 - after sys.path setup
...
def generate_test_portal(
    ...
    if multi_domain:
        ...
        framework = create_test_framework(
            num_properties=10, num_processes=10, num_perspectives=10, num_relationships=50
        )
...
def main():
    ...
    parser.add_argument("--output-dir", type=str, default=None, help="Directory to save the portal")
    parser.add_argument(
        "--multi-domain", action="store_true", help="Use a multi-domain test framework"
    )
    parser.add_argument(
        "--no-dropdown", action="store_true", help="Do not include dataset dropdown"
    )
    parser.add_argument(
        "--no-selector", action="store_true", help="Do not include component selector"
    )
    parser.add_argument(
        "--no-browser", action="store_true", help="Do not open the portal in the browser"
    )
