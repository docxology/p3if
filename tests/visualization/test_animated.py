...
class TestAnimatedDimensions(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Add some test data
        prop = Property(
            name="Test Property", description="A test property for animation", domain="test"
        )
        proc = Process(
            name="Test Process", description="A test process for animation", domain="test"
        )
        persp = Perspective(
            name="Test Perspective",
            description="A test perspective for animation",
            domain="test",
            viewpoint="test",
        )
    ...
    def test_dimension_animator_creation(self):
        ...
        animator = DimensionAnimator(name="test_animator", framework_data={"test": "data"})
    ...
    def test_save_animation(self, mock_file):
        ...
        from p3if.visualization.animated.animated_dimensions import (
            DimensionAnimator,
            save_animation,
        )
...
class TestAnimationVisualizations(unittest.TestCase):
    ...
    def setUp(self):
        ...
        prop = Property(
            name="Test Property", description="Small framework test property", domain="test"
        )
        proc = Process(
            name="Test Process", description="Small framework test process", domain="test"
        )
        persp = Perspective(
            name="Test Perspective",
            description="Small framework test perspective",
            domain="test",
            viewpoint="test",
        )
        ...
        for i in range(5):
            prop = Property(
                name=f"Property {i}", description=f"Large framework property {i}", domain="test"
            )
            proc = Process(
                name=f"Process {i}", description=f"Large framework process {i}", domain="test"
            )
            persp = Perspective(
                name=f"Perspective {i}",
                description=f"Large framework perspective {i}",
                domain="test",
                viewpoint="test",
            )
    ...
    def test_generate_animation_visualizations(self, mock_logger):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                generate_animation_visualizations(
                    self.small_framework, self.large_framework, session_path
                )
    ...
    def test_animation_functions_exist(self):
        ...
        self.assertTrue(
            hasattr(animation_visualizations, "_create_relationship_dynamics_animation")
        )
        self.assertTrue(
            hasattr(animation_visualizations, "_create_component_interaction_animation")
        )
