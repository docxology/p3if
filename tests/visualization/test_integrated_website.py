...
class TestIntegratedWebsite(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.frameworks = {
            "small": create_test_framework(**DEFAULT_TEST_SETTINGS["FRAMEWORK"]["SMALL"]),
            "medium": create_test_framework(**DEFAULT_TEST_SETTINGS["FRAMEWORK"]["MEDIUM"]),
            "large": create_large_test_framework(
                **DEFAULT_TEST_SETTINGS["FRAMEWORK"]["LARGE"], num_domains=3
            ),
    ...
    def test_website_export_functionality(self):
        ...
        self.portals["medium"].generate_portal(
            output_file=output_file, title="Export Functionality Test", include_export_buttons=True
        )
