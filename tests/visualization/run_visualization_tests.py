...
# Import test modules
from tests.visualization.test_interactive import (  # noqa: E402 - after sys.path setup
    TestInteractiveVisualizer,
)  # noqa: E402 - import after sys.path setup
from tests.visualization.test_portal import (  # noqa: E402 - after sys.path setup
    TestVisualizationPortal,
)  # noqa: E402 - import after sys.path setup
from tests.visualization.test_dashboard import (  # noqa: E402 - after sys.path setup
    TestDashboardGenerator,
)  # noqa: E402 - import after sys.path setup
from tests.visualization.test_integrated_website import (  # noqa: E402 - after sys.path setup
    TestIntegratedWebsite,
)  # noqa: E402 - import after sys.path setup
from tests.visualization.test_base import TestVisualizer  # noqa: E402 - import after sys.path setup
...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("visualization_tests.log")],
)
...
class OutputPathTrackingTestResult(unittest.TextTestResult):
    ...
    def _log_outputs(self, base_path, test):
        ...
        try:
            ...
            logger.info(
                f"Output directory for {test.__class__.__name__}.{test._testMethodName}: {base_path}"
            )
...
def run_selected_tests(
    interactive=True, portal=True, dashboard=True, integrated=True, base=True, save_report=True
):
    """
...
def main():
    ...
    parser.add_argument(
        "--interactive", action="store_true", help="Run only interactive visualization tests"
    )
    parser.add_argument("--portal", action="store_true", help="Run only visualization portal tests")
    parser.add_argument(
        "--dashboard", action="store_true", help="Run only dashboard generator tests"
    )
    parser.add_argument(
        "--integrated", action="store_true", help="Run only integrated website tests"
    )
    parser.add_argument("--base", action="store_true", help="Run only base visualizer tests")
    parser.add_argument("--all", action="store_true", help="Run all visualization tests")
    parser.add_argument("--report", action="store_true", help="Save a test report to file")
    ...
    run_all = args.all or not (
        args.interactive or args.portal or args.dashboard or args.integrated or args.base
    )
