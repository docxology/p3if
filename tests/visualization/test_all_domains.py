...
try:
    ...
    from p3if.visualization.portals import VisualizationPortal
...
class TestAllDomains(unittest.TestCase):
    ...
    def test_all_domains_can_be_loaded(self):
        ...
        for domain_name in self.all_domains:
            ...
            self.assertIn("name", domain_info, f"Domain {domain_name} missing 'name' field")
            self.assertIn("counts", domain_info, f"Domain {domain_name} missing 'counts' field")
            # Check that counts contains the expected structure
            counts = domain_info["counts"]
            self.assertIn("properties", counts)
            self.assertIn("processes", counts)
            self.assertIn("perspectives", counts)
    ...
    def test_all_domains_can_generate_data(self):
        ...
        for domain_name in self.all_domains:
            ...
            self.assertGreater(
                len(framework._patterns), 0, f"No patterns generated for domain: {domain_name}"
            )
            self.assertGreater(
                len(framework._relationships),
                0,
                f"No relationships generated for domain: {domain_name}",
            )
    ...
    def test_all_domains_can_be_visualized(self):
        ...
        for domain_name in self.all_domains:
            ...
            try:
                ...
                self.assertTrue(
                    result_file.exists(), f"Visualization failed for domain: {domain_name}"
                )
                self.assertGreater(
                    result_file.stat().st_size, 0, f"Empty visualization for domain: {domain_name}"
                )
                ...
                with open(result_file, "r") as f:
                    content = f.read()
                    self.assertIn(
                        "3d-cube-container", content, f"3D cube missing for domain: {domain_name}"
                    )
                    self.assertIn(
                        "network-container", content, f"Network missing for domain: {domain_name}"
                    )
                    self.assertIn(
                        "matrix-container", content, f"Matrix missing for domain: {domain_name}"
                    )
    ...
    def test_multiple_domains_can_be_visualized_together(self):
        ...
        self.generator.generate_cross_domain_connections(
            framework=framework, num_connections=5  # Small number of connections for testing
        )

        # Check that the framework contains data
        self.assertGreater(len(framework._patterns), 0, "No patterns generated for multi-domain")
        self.assertGreater(
            len(framework._relationships), 0, "No relationships generated for multi-domain"
        )
        ...
        for domain in test_domains:
            ...
            datasets.append(
                {"id": domain_info.get("id", domain.lower().replace(" ", "_")), "name": domain}
            )
        ...
        try:
            ...
            with open(result_file, "r") as f:
                ...
                self.assertIn(
                    "dataset-selector",
                    content,
                    "Dataset selector missing in multi-domain visualization",
                )
                self.assertIn(
                    "3d-cube-container", content, "3D cube missing in multi-domain visualization"
                )
                self.assertIn(
                    "network-container", content, "Network missing in multi-domain visualization"
                )
                self.assertIn(
                    "matrix-container", content, "Matrix missing in multi-domain visualization"
                )
                self.assertIn(
                    "dashboard-container",
                    content,
                    "Dashboard missing in multi-domain visualization",
                )

                # Check that all domain names are in the content
                for domain in test_domains:
                    self.assertIn(
                        domain, content, f"Domain {domain} missing in multi-domain visualization"
                    )
