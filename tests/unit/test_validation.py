...
class TestConstraintManager(unittest.TestCase):
    ...
    def test_check_constraints(self):
        ...
        # Test with pattern that has description (since description is now required, all patterns have it)
        prop_with_desc = Property(name="Test", domain="test", description="Test property")
        violations = self.manager.check_constraints(prop_with_desc)
        self.assertEqual(len(violations), 0)
...
class TestValidationRules(unittest.TestCase):
    ...
    def test_dimension_balance_rule(self):
        ...
        for i in range(10):
            prop = Property(name=f"Property {i}", domain="test", description=f"Test property {i}")
            self.framework.add_pattern(prop)
